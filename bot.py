...
def read_fixos(spreadsheet_id: str) -> list:
    ...
    for row in rows[1:]:
        if len(row) >= 4 and row[0]:
            items.append({
                "nome": row[0], "valor": float(row[1]) if row[1] else 0,
...
def read_parcelas(spreadsheet_id: str) -> list:
    ...
    for row in rows[1:]:
        if len(row) >= 4 and row[0]:
            items.append({
                "nome": row[0], "total": float(row[1]) if row[1] else 0,
...
def read_metas(spreadsheet_id: str) -> list:
    ...
    for row in rows[1:]:
        if len(row) >= 2 and row[0]:
            items.append({
                "nome": row[0], "meta": float(row[1]) if row[1] else 0,
                "atual": float(row[2]) if len(row)>2 and row[2] else 0,
...
def main_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📄 Relatório", callback_data="menu_relatorio"),
         InlineKeyboardButton("🏆 Conquistas", callback_data="menu_conquistas")],
    ])
...
def get_level_from_streak(streak: int) -> tuple:
    if streak >= 60: return ("💎 Diamante", "diamante")
    if streak >= 30: return ("🟡 Ouro", "ouro")
    if streak >= 10: return ("🔶 Prata", "prata")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user.get("onboarding_done"):
        ...
        await update.message.reply_text(
            ...
            f"🔥 Streak: {user.get('streak',0)} dias | Nível: {level_icon} {level_name}\n\n"
            "O que deseja fazer?",
...
async def onboarding_goal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "Comandos novos: */fixo*, */parcela*, */meta*, */busca*, */relatorio*",
            parse_mode="Markdown", reply_markup=main_menu_keyboard()
        )
        return ConversationHandler.END
...
async def salvar_gasto(update: Update, context, obs: str):
        ...
        try:
            ...
            if cartao:
                ...
                if existing:
                    ...
                    cur = float(row_data[2]) if len(row_data) > 2 and row_data[2] else 0.0
    ...
    # Atualizar streak e gastos_count
    ...
    # Calcular nível
    level_icon, level_name = get_level_from_streak(streak)

    upsert_user(user_id, last_gasto_date=today, streak=streak, level=level_name, gastos_count=gastos_count)
    ...
    if gastos_count >= 10:
        unlock_achievement(user_id, "gastos_10")
    if gastos_count >= 100:
        unlock_achievement(user_id, "gastos_100")
...
async def cmd_resumo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in data[1:]:
        if len(r) > 5 and r[2] and r[5]:
            cats[r[2]] = cats.get(r[2], 0) + float(r[5])
...
async def execute_busca(update: Update, context: ContextTypes.DEFAULT_TYPE, query_text: str):
    ...
    total = sum(float(r[5]) for r in results if r[5])
...
async def cmd_relatorio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    necessarios = sum(float(r[5]) for r in data[1:] if len(r) > 7 and r[7] == "SIM" and r[5])
...
async def cmd_conquistas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append("\n🎯 *Próximas conquistas:*")
    if "streak_7" not in unlocked_keys:
        lines.append(f"  🔒 {ACHIEVEMENT_NAMES['streak_7']}")
    if "streak_30" not in unlocked_keys and "streak_7" in unlocked_keys:
        lines.append(f"  🔒 {ACHIEVEMENT_NAMES['streak_30']}")
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "menu_gasto":
        await query.edit_message_text("💸 Use */gasto* ou */g <valor> <desc> <categoria> [pagamento]*", parse_mode="Markdown")
    elif cmd == "menu_resumo":
        await query.edit_message_text("📊 Use */resumo* para ver o resumo financeiro.", parse_mode="Markdown")
    elif cmd == "menu_fixos":
        user = get_user(update.effective_user.id)
        fixos = read_fixos(user["spreadsheet_id"])
        if not fixos:
            await query.edit_message_text("📌 Nenhum fixo. Use */fixo add*", parse_mode="Markdown"); return
        ativos = [f for f in fixos if f.get("ativo",True)]
        total = sum(f["valor"] for f in ativos)
        lines = [f"📌 *Gastos Fixos ({len(ativos)})*", f"💰 Total: R$ {total:,.2f}\n"]
        for f in ativos:
            lines.append(f"• *{f['nome']}* — R$ {f['valor']:,.2f} — dia {f['dia']}" if f.get("dia") else f"• *{f['nome']}* — R$ {f['valor']:,.2f}")
        await query.edit_message_text("\n".join(lines), parse_mode="Markdown")
        ...
        await query.edit_message_text("🎯 Use */meta* para ver ou */meta add* para criar metas.", parse_mode="Markdown")
    elif cmd == "menu_busca":
        ...
        await query.edit_message_text("📄 Use */relatorio* para ver o relatório financeiro completo.", parse_mode="Markdown")
    elif cmd == "menu_conquistas":
        await query.edit_message_text("🏆 Use */conquistas* para ver suas conquistas e nível.", parse_mode="Markdown")
    else:
        await query.edit_message_text("✅ Funcionalidade em desenvolvimento. Use os comandos diretos.")
...
async def post_init(application):
    ...
    await application.bot.set_my_commands([
        ...
        ("conquistas", "Suas conquistas e nível"),
